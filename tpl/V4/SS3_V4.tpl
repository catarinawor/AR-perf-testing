...
//  ProgLabel_2.7.4 read Length composition data
 ...
 END_CALCS
  ...
  3darray obs_l_all(1,2,1,Ntypes,1,nlen_bin)  //  for the sum of all length comp data
...
//  ProgLabel_3.0   read forecast.ss
 ...
 END_CALCS
  ...
  int N_Fcast_catches
  int Fcast_InputCatch_Basis  //  2=dead catch; 3=retained catch;  99=F  (biomass vs numbers will match catchunits(fleet)
...
//  matrix Fcast_RelF(1,nseas,1,Nfleet)
 ...
 END_CALCS
  ...
  matrix Fcast_InputCatch(k1,y,1,Nfleet)
  init_matrix Fcast_InputCatch_1(1,N_Fcast_catches,1,4)
...
//  ProgLabel_4.5.1  create time constants for growth
  ...
  number AFIX_delta;
  number AFIX_plus;
  number Age_K_1;  //  min age for age-specific K
  number Age_K_2;  //  max age for age-specific K
  !! k=0;
...
//  ProgLabel_5.1.1  create MGparm vector and associated arrays
  ...
  matrix MGparm_trend(1,N_MGparm_trend,styr,YrMax);
  matrix MGparm_block_val(1,N_MGparm,styr,YrMax);
...
//  vector S1(0,1);
  ...
  vector exp_rec(styr-3,YrMax) //expected value for recruitment (deterministic)
  vector pred_rec(styr-3,YrMax) //predicted value for recruitment (stochastic)
  vector use_rec(styr-3,YrMax) //value used to calc Numbers at age
...
//  ProgLabel_5.1.3  create F parameters and associated arrays and constants
 ...
 LOCAL_CALCS
  if(F_Method==2)    // continuous F
    ...
    {k=-1;}
 END_CALCS
  init_bounded_number_vector F_rate(1,k,0.,50.,have_catch)
...
//  ProgLabel_5.2  Create arrays for storing derived selectivity quantities for use in mortality calculations
  ...
  3darray save_sp_len(styr,endyr+1,1,2*Ntypes,1,50);     // use to output selex parm values after adjustment
...
//  ProgLabel_11.1 reduce F_ballpark_lambda
  if(j_phase>1)
  {
...
//  ProgLabel_11.2 for Fmethod=2, set parameter values (F_rate) equal to Hrate array fromcalculated using hybrid method in previous phase
    if(F_Method==2)
    {
...
//*********************************************************************
//  ProgLabel_FUNCTION 14 get_MGsetup
//  ProgLabel_14.   FUNCTION get_MGsetup:  apply time-varying factors this year to the MG parameters to create mgp_adj vector
...
//  ProgLabel_15.  FUNCTION get_growth1  Contains some seasonal and CV_growth biology factors that cannot be time-varying
...
//  ProgLabel_16.  FUNCTION get_growth2 (do seasonal growth calculations for a selected year)
...
//                  if(docheckup==1) checkup<<" growth "<<g<<" "<<gp<<" Lmin: "<<Lmin(gp)<<" Linf: "<<L_inf(gp)<<" VBK: "<<VBK(gp)<<endl<<Ave_Size(styr,0,g)(0,min(6,nages))<<endl;
                  break;
                }
...
//                  if(docheckup==1) checkup<<" growth "<<g<<" "<<gp<<" Lmin: "<<Lmin(gp)<<" Linf: "<<LinfR<<" VBK: "<<VBK(gp)<<endl<<Ave_Size(styr,0,g)(0,min(6,nages))<<endl;
                  break;
                }
              }
...
//  ProgLabel_17.  FUNCTION get_natmort
...
//  ProgLabel_18.  FUNCTION get_recr_distribution
...
//  ProgLabel_FUNCTION 19 get_wtlen
//  ProgLabel_19.  FUNCTION get_wtlen, maturity, fecundity, hermaphroditism
FUNCTION get_wtlen
...
//  ProgLabel_20.  FUNCTION get_migration
...
//  ProgLabel_21.  FUNCTION get_saveGparm
...
//  ProgLabel_22. FUNCTION get_selectivity
...
//    Do size-Retention ---// ProgLabel_070----------------------------------------------------------------
...
//  ProgLabel_23. FUNCTION get_initial_conditions
...
//  ProgLabel_24. FUNCTION  get_time_series
...
// THORSON, thorson, Thorson changes
...
//  ProgLabel_26  FUNCTION Process_STDquant
...
//  ProgLabel_27  FUNCTION Check_Parm
...
//  ProgLabel_28  FUNCTION Report_Parm
...
//  ProgLabel_29  FUNCTION Get_Prior
...
//  ProgLabel_30 FUNCTION   Do_Equil_Calc
...
// ProgLabel_31 FUNCTION Make_AgeLength_Key
...
// ProgLabel_32 FUNCTION  Make_FishSelex
...
// ********************************************************************************************************
//  ProgLabel_FUNCTION 33 get_posteriors
//  ProgLabel_33 FUNCTION get_posteriors  (MCMC eval)
...
// ***************************************************************************************
//  ProgLabel_FUNCTION 34 Get_Benchmarks
//  ProgLabel_34 FUNCTION Get_Benchmarks(); (Find Fspr, MSY)
...
//  ProgLabel_35  FUNCTION Get_Forecast
                ...
                if (fleet_area(f)==p && Fcast_RelF_Use(s,f)>0.0)
                {
                  ...
                  if(Do_F_tune(t,f)==1)  // have an input catch, so get expected catch from F and Z
                  {
                    if(ABC_Loop==2)  //  tune to input catch
                    {
                      ...
                      if(Tune_F<3)
                      {
//                        report5<<t<<" area "<<p<<" tune "<<Tune_F<<" flt "<<f<<" Hrate "<<Hrate(f,t)<<" old "<<H_old(f)<<" "<<C_old(f)<<" temp "<<H_temp(f)<<" "<<C_temp(f)<<" "<<Fcast_InputCatch(t,f);
                        C_old(f)=C_temp(f);
                        H_old(f)=H_temp(f);
...
//                        report5<<" new "<<Hrate(f,t)<<endl;
                      ...
                      {
//                        report5<<t<<" area "<<p<<" "<<Tune_F<<" "<<f<<" "<<Hrate(f,t)<<" old "<<H_old(f)<<" "<<C_old(f)<<" temp "<<H_temp(f)<<" "<<C_temp(f)<<" "<<Fcast_InputCatch(t,f);
...
//  ProgLabel_36 FUNCTION  Write Summary output
...
//  ProgLabel_37 FUNCTION write_rebuilder_output
...
//  ProgLabel_38 FUNCTION write_nudata
...
//***********************************************************************************
//  ProgLabel_FUNCTION 39 write_nucontrol
//  ProgLabel_39 FUNCTION write_nucontrol  write new control file
...
//  ProgLabel_40 FUNCTION write_bigoutput
...
//  ProgLabel_41  FUNCTION write_Bzero_output
...
//  ProgLabel_42 FUNCTION Join_Fxn
...
//  ProgLabel_43 FUNCTION Spawn_Recr
...
//  ProgLabel_44 FUNCTION Equil_Spawn_Recr_Fxn
//FUNCTION dvar_vector Equil_Spawn_Recr_Fxn(const prevariable& SPR_temp)
...
//  ProgLabel_44.1.6  3 parameter survival based
      case 7:  // survival
      {
...
//  ProgLabel_FUNCTION 45 get_age_age
//  ProgLabel_45 get_age_age
FUNCTION void get_age_age(const int Keynum, const int AgeKey_StartAge, const int AgeKey_Linear1, const int AgeKey_Linear2)
  ...
  {
//  Proglabel_45.1 set age_err_parm to mgp_adj, so can be time-varying according to MGparm options
    for(a=1;a<=7;a++)
...
//  Proglabel_45.3 calc ageing bias
      if(AgeKey_Linear1==0)
      {
...
//  Proglabel_45.4 calc ageing variance
      if(AgeKey_Linear2==0)
      {
...
//  Proglabel_45.5 calc distribution of age' for each age
   for (a=0; a<=nages;a++)
    {
