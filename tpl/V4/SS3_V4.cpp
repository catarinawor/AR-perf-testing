...
{
version_info+="SS-V3.24f-safe;_08/03/2012;_Stock_Synthesis_by_Richard_Methot_(NOAA)_using_ADMB_10.1";
version_info_short+="#V3.24f";
 No_Report=0;
 Ncycle=3;
...
echoinput<<SPR_reporting<<"  SPR_reporting"<<endl;
  ...
  F_reporting_ages_R.allocate(1,k,"F_reporting_ages_R");
  F_reporting_ages.allocate(1,2);
  if (F_reporting==4)
  {
    F_reporting_ages=F_reporting_ages_R;
    echoinput<<F_reporting_ages<<"  F_reporting_ages"<<endl;
...
echoinput<<"For Kobe plot, set depletion_basis=2; depletion_level=1.0; F_reporting=your choose; F_report_basis=2"<<endl;
 ...
 if(Turn_off_phase<=0) {dummy_phase=0;} else {dummy_phase=-6;}
  ...
  prof_var.allocate(1,prof_junk+runnumber*N_prof_var,"prof_var");
...
echoinput<<nages<<" nages is maxage "<<endl;
  ...
  frac_ages.allocate(0,nages);
  years.allocate(styr,endyr);
  r_years.allocate(styr,endyr);
...
echoinput<<" catch as read "<<endl<<catch_bioT<<endl;
 ...
 k=Nfleet*(TimeMax-styr+1);
  ...
  nobs_cr_rd.allocate("nobs_cr_rd");
  echoinput<<nobs_cr_rd<<" nobs_survey "<<endl;
  if(nobs_cr_rd>0) {k=3; j=Ntypes;} else {k=0; j=Ntypes;}
  cr_units_rd.allocate(1,Ntypes,1,k,"cr_units_rd");
  cr_units.allocate(1,j);
  cr_errtype.allocate(1,j);
  if(k>0)
  {
...
if(nobs_cr_rd>0) echoinput<<" indexdata "<<endl<<indexdata<<endl;
  ...
  yr_cr_s.allocate(1,Ntypes,1,nyr_cr);
  yr_cr_use.allocate(1,Ntypes,1,nyr_cr);
  obs_cr.allocate(1,Ntypes,1,nyr_cr);
  Ln_obs_cr.allocate(1,Ntypes,1,nyr_cr);
  se_cr_obs.allocate(1,Ntypes,1,nyr_cr);
  vul_bio.allocate(1,Ntypes,1,nyr_cr);
  suprper_cr1.allocate(1,Ntypes,1,N_suprper_cr);
  suprper_cr2.allocate(1,Ntypes,1,N_suprper_cr);
  suprper_cr_sampwt.allocate(1,Ntypes,1,nyr_cr);
    N_suprper_cr=0;
    nyr_cr=0;
    in_superperiod=0;
    if(nobs_cr>0)  // proceed if any data in yr range
    {
      for(i=1;i<=nobs_cr_rd;i++)  // loop all, including those out of yr range
      {
        ...
        if(y>=styr && y<=retro_yr)
        {
          ...
          else if(in_superperiod==1)  // end superiod
           {suprper_cr2(f,N_suprper_cr(f))=j; in_superperiod=0;}
        }
      }
  ...
  Ndisc_fleets.allocate("Ndisc_fleets");
  ...
  disc_units_rd.allocate(1,Ndisc_fleets,1,3,"disc_units_rd");
  disc_units.allocate(1,j);
  disc_errtype.allocate(1,j);
  disc_errtype_r.allocate(1,j);
 ...
 if(nobs_disc_rd>0) echoinput<<" discarddata "<<endl<<discdata<<endl;
  ...
  N_suprper_disc.allocate(1,Ntypes);
  nobs_disc=0;
  nyr_disc=0;
  N_suprper_disc=0;
  if(nobs_disc_rd>0)
  ...
  yr_disc_s.allocate(1,Ntypes,1,nyr_disc);
  yr_disc_use.allocate(1,Ntypes,1,nyr_disc);
  obs_disc.allocate(1,Ntypes,1,nyr_disc);
  cv_disc.allocate(1,Ntypes,1,nyr_disc);
  sd_disc.allocate(1,Ntypes,1,nyr_disc);
  suprper_disc1.allocate(1,Ntypes,1,N_suprper_disc);
  suprper_disc2.allocate(1,Ntypes,1,N_suprper_disc);
  suprper_disc_sampwt.allocate(1,Ntypes,1,nyr_disc);
...
if(nobsl_rd>0) echoinput<<" first lencomp obs "<<endl<<lendata(1)<<endl<<" last obs"<<endl<<lendata(nobsl_rd)<<endl;;
  Nobs_l.allocate(1,Ntypes);
  N_suprper_l.allocate(1,Ntypes);
  ...
  yr_l_t.allocate(1,Ntypes,1,Nobs_l);
  yr_l_y.allocate(1,Ntypes,1,Nobs_l);
  obs_l.allocate(1,Ntypes,1,Nobs_l,1,nlen_bin2);
  obs_l_all.allocate(1,2,1,Ntypes,1,nlen_bin);
  nsamp_l.allocate(1,Ntypes,1,Nobs_l);
  nsamp_l_read.allocate(1,Ntypes,1,Nobs_l);
  gen_l.allocate(1,Ntypes,1,Nobs_l);
  mkt_l.allocate(1,Ntypes,1,Nobs_l);
  header_l.allocate(1,Ntypes,1,Nobs_l,0,3);
  tails_l.allocate(1,Ntypes,1,Nobs_l,1,4);
  tails_w.allocate(1,4);
  suprper_l1.allocate(1,Ntypes,1,N_suprper_l);
  suprper_l2.allocate(1,Ntypes,1,N_suprper_l);
  suprper_l_sampwt.allocate(1,Ntypes,1,Nobs_l);
  N_suprper_l=0;
  Nobs_l=0;
  in_superperiod=0;
    ...
    for(i=1;i<=nobsl_rd;i++)   //  loop all observations to find those for this fleet/time
    {
      ...
      if(y>=styr && y<=retro_yr)
      {
        ...
        if(f==floop)
        {
          ...
          if(gender==2 && gen_l(f,j)>=2)
          {
            for(z=1;z<=nlen_bin;z++)
            {
              obs_l_all(1,f,z)+=obs_l(f,j,nlen_bin+z);  //  males
            }
          }
        }
      }
     ...
     for(f=1;f<=Ntypes;f++)
     {
       if(Nobs_l(f)>0)
       {
         obs_l_all(1,f)/=sum(obs_l_all(1,f));
       }
       else
       {
         obs_l_all(1,f)=float(1./nlen_bin);
       }
...
echoinput<<N_ageerr<<" N age error defs "<<endl;
  age_err_rd.allocate(1,N_ageerr,1,2,0,nages,"age_err_rd");
  Use_AgeKeyZero=0;
  if(N_ageerr>0)
  {
...
if(nobsa_rd>0) echoinput<<" first agecomp obs "<<endl<<agedata(1)<<endl<<" last obs"<<endl<<agedata(nobsa_rd)<<endl;;
  Nobs_a.allocate(1,Ntypes);
  N_suprper_a.allocate(1,Ntypes);
  for (b=1;b<=n_abins;b++)
  {
  ...
  yr_a_t.allocate(1,Ntypes,1,Nobs_a);
  yr_a_y.allocate(1,Ntypes,1,Nobs_a);
  obs_a.allocate(1,Ntypes,1,Nobs_a,1,gender*n_abins);
  obs_a_all.allocate(1,2,1,Ntypes,1,n_abins);
  nsamp_a.allocate(1,Ntypes,1,Nobs_a);
  nsamp_a_read.allocate(1,Ntypes,1,Nobs_a);
  ageerr_type_a.allocate(1,Ntypes,1,Nobs_a);
  gen_a.allocate(1,Ntypes,1,Nobs_a);
  mkt_a.allocate(1,Ntypes,1,Nobs_a);
  Lbin_filter.allocate(1,Ntypes,1,Nobs_a,1,nlength2);
  use_Lbin_filter.allocate(1,Ntypes,1,Nobs_a);
  Lbin_lo.allocate(1,Ntypes,1,Nobs_a);
  Lbin_hi.allocate(1,Ntypes,1,Nobs_a);
  tails_a.allocate(1,Ntypes,1,Nobs_a,1,4);
  header_a.allocate(1,Ntypes,1,Nobs_a,0,9);
  suprper_a_sampwt.allocate(1,Ntypes,1,Nobs_a);
  suprper_a1.allocate(1,Ntypes,1,N_suprper_a);
  suprper_a2.allocate(1,Ntypes,1,N_suprper_a);
  Lbin_filter=1.;
...
if(nobs_ms_rd>0) echoinput<<" first size-at-age obs "<<endl<<sizeagedata(1)<<endl<<" last obs"<<endl<<sizeagedata(nobs_ms_rd)<<endl;;
  Nobs_ms.allocate(1,Ntypes);
  N_suprper_ms.allocate(1,Ntypes);
  Nobs_ms=0;
  N_suprper_ms=0;
  if(nobs_ms_rd>0)
  ...
  yr_ms_t.allocate(1,Ntypes,1,Nobs_ms);
  yr_ms_y.allocate(1,Ntypes,1,Nobs_ms);
  obs_ms.allocate(1,Ntypes,1,Nobs_ms,1,n_abins2);
  obs_ms_n.allocate(1,Ntypes,1,Nobs_ms,1,n_abins2);
  obs_ms_n_read.allocate(1,Ntypes,1,Nobs_ms,1,n_abins2);
  ageerr_type_ms.allocate(1,Ntypes,1,Nobs_ms);
  gen_ms.allocate(1,Ntypes,1,Nobs_ms);
  mkt_ms.allocate(1,Ntypes,1,Nobs_ms);
  use_ms.allocate(1,Ntypes,1,Nobs_ms);
  header_ms.allocate(1,Ntypes,1,Nobs_ms,0,7);
  suprper_ms_sampwt.allocate(1,Ntypes,1,Nobs_ms);
  suprper_ms1.allocate(1,Ntypes,1,N_suprper_ms);
  suprper_ms2.allocate(1,Ntypes,1,N_suprper_ms);
   Nobs_ms=0;
...
echoinput<<N_envdata<<" N_envdata "<<endl;
  ...
  SzFreq_Nmeth.allocate("SzFreq_Nmeth");
echoinput<<SzFreq_Nmeth<<" N sizefreq methods to read "<<endl;
  SzFreq_HaveObs.allocate(1,Ntypes,1,SzFreq_Nmeth,styr,TimeMax,0,2);
  SzFreq_HaveObs2.allocate(1,SzFreq_Nmeth,styr,TimeMax);
  SzFreq_Nbins.allocate(1,SzFreq_Nmeth,"SzFreq_Nbins");
if(SzFreq_Nmeth>0) echoinput<<SzFreq_Nbins<<" Sizefreq N bins per method"<<endl;
  SzFreq_units.allocate(1,SzFreq_Nmeth,"SzFreq_units");
if(SzFreq_Nmeth>0) echoinput<<SzFreq_units<<" Sizetfreq units(bio/num) per method"<<endl;
  SzFreq_scale.allocate(1,SzFreq_Nmeth,"SzFreq_scale");
if(SzFreq_Nmeth>0) echoinput<<SzFreq_scale<<" Sizefreq scale(kg/lbs/cm/inches) per method"<<endl;
  SzFreq_mincomp.allocate(1,SzFreq_Nmeth,"SzFreq_mincomp");
if(SzFreq_Nmeth>0) echoinput<<SzFreq_mincomp<<" Sizefreq mincomp per method "<<endl;
  SzFreq_nobs.allocate(1,SzFreq_Nmeth,"SzFreq_nobs");
if(SzFreq_Nmeth>0) echoinput<<SzFreq_nobs<<" Sizefreq N obs per method"<<endl;
  SzFreq_Nbins_seas_g.allocate(1,SzFreq_Nmeth*nseas);
  SzFreq_Nbins3.allocate(1,SzFreq_Nmeth);
  g=0;
  if(SzFreq_Nmeth>0)
  {
  ...
  SzFreq_bins1.allocate(1,SzFreq_Nmeth,1,SzFreq_Nbins,"SzFreq_bins1");
if(SzFreq_Nmeth>0) echoinput<<" SizeFreq bins-raw "<<endl<<SzFreq_bins1<<endl;
  SzFreq_bins.allocate(1,SzFreq_Nmeth,1,SzFreq_Nbins3);
  SzFreq_bins2.allocate(1,SzFreq_Nmeth,0,SzFreq_Nbins3+1);
  SzFreq_Omit_Small.allocate(1,SzFreq_Nmeth);
  if(SzFreq_Nmeth>0)
  {
  for(k=1;k<=SzFreq_Nmeth;k++)
  {
  ...
  SzFreq_Setup.allocate(1,SzFreq_totobs);
  SzFreq_Setup2.allocate(1,SzFreq_totobs);
  if(SzFreq_Nmeth>0)
  {
  g=0;
  for(k=1;k<=SzFreq_Nmeth;k++)
  ...
  SzFreq_obs1.allocate(1,SzFreq_totobs,1,SzFreq_Setup,"SzFreq_obs1");
if(SzFreq_totobs>0) echoinput<<" first sizefreq obs "<<endl<<SzFreq_obs1(1)<<endl<<" last obs"<<endl<<SzFreq_obs1(SzFreq_totobs)<<endl;;
  SzFreq_obs_hdr.allocate(1,SzFreq_totobs,1,8);
  SzFreq_sampleN.allocate(1,SzFreq_totobs);
  SzFreq_effN.allocate(1,SzFreq_totobs);
  SzFreq_eachlike.allocate(1,SzFreq_totobs);
  SzFreq_obs.allocate(1,SzFreq_totobs,1,SzFreq_Setup2);
  SzFreq_LikeComponent.allocate(1,Ntypes,1,SzFreq_Nmeth);
  SzFreq_N_Like=0;
  if(SzFreq_Nmeth>0)
  {
  ...
  SzFreq_like_base.allocate(1,SzFreq_N_Like);
  suprper_SzFreq_start.allocate(1,N_suprper_SzFreq);
  suprper_SzFreq_end.allocate(1,N_suprper_SzFreq);
  suprper_SzFreq_sampwt.allocate(1,SzFreq_totobs);
  if(SzFreq_Nmeth>0)
  {
...
echoinput<<BTGT_target<<" BTGT_target "<<endl;
  Bmark_Yr.allocate(1,6);
  Bmark_t.allocate(1,2);
  Bmark_Yr_rd.allocate(1,6,"Bmark_Yr_rd");
  Bmark_RelF_Basis.allocate("Bmark_RelF_Basis");
...
echoinput<<Do_Forecast<<" Do_Forecast "<<endl;
  Fcast_Input.allocate(1,22);
  Fcast_yr.allocate(1,4);
  Fcast_Loop_Control.allocate(1,5);
  if(Do_Forecast==0)
  {
    k=0;
  ...
  Fcast_Input_rd.allocate(1,k,"Fcast_Input_rd");
  if(Do_Forecast>0)
  {
  ...
  {z=0;}
  Fcast_RelF_Input.allocate(1,z,1,Nfleet,"Fcast_RelF_Input");
  if(Fcast_RelF_Basis==2)
  {
  ...
  Fcast_MaxFleetCatch.allocate(1,k*Nfleet,"Fcast_MaxFleetCatch");
  Max_Fcast_Catch.allocate(1,k*pop,"Max_Fcast_Catch");
  Allocation_Fleet_Assignments.allocate(1,k*Nfleet,"Allocation_Fleet_Assignments");
  ...
  Fcast_Catch_Allocation.allocate(1,Fcast_Catch_Allocation_Groups,"Fcast_Catch_Allocation");
  if(Do_Forecast>0)
  {
    k=2;
  ...
  {k=0;}
  more_Fcast_input.allocate(1,k,"more_Fcast_input");
  if(k>0)
  {
    N_Fcast_catches=more_Fcast_input(1);
    Fcast_InputCatch_Basis=more_Fcast_input(2);
    echoinput<<N_Fcast_catches<<" N_Fcast_input_catches "<<endl;
    echoinput<<Fcast_InputCatch_Basis<<" # basis for input Fcast catch:  2=dead catch; 3=retained catch; 99=input Hrate(F) (bio/num units are from fleetunits; note new codes in SSV3.20)"<<endl;
    k1 = styr+(endyr-styr)*nseas-1 + nseas + 1;
    y=k1+(N_Fcast_Yrs)*nseas-1;
    if(N_Fcast_catches>0) echoinput<<"Now read "<<N_Fcast_catches<<" of fixed forecast catches (yr, seas, fleet, catch) "<<endl;
  }
  else
  {
    k1=1;
  ...
  Fcast_InputCatch.allocate(k1,y,1,Nfleet);
  Fcast_InputCatch_1.allocate(1,N_Fcast_catches,1,4,"Fcast_InputCatch_1");
 if(N_Fcast_catches>0) echoinput<<" Fcast_catches "<<endl<<Fcast_InputCatch_1<<endl;
  if(Do_Forecast>0)
  {
    for(t=k1;t<=y;t++)
    for(f=1;f<=Nfleet;f++)
    {Fcast_InputCatch(t,f)=-1;}
    if(N_Fcast_catches>0)
    {
      for(i=1;i<=N_Fcast_catches;i++)
      {
        y=Fcast_InputCatch_1(i,1); s=Fcast_InputCatch_1(i,2); f=Fcast_InputCatch_1(i,3);
        if(y>endyr && y<=YrMax && f<=Nfleet)
        {
          t=styr+(y-styr)*nseas +s-1;
          Fcast_InputCatch(t,f)=Fcast_InputCatch_1(i,4);
...
echoinput<<N_GP<<" N growth patterns "<<endl;
  N_platoon.allocate("N_platoon");
echoinput<<N_platoon<<"  N platoons (1, 3 or 5)"<<endl;
  ishadow.allocate(1,N_platoon);
  shadow.allocate(1,N_platoon);
  submorphdist.allocate(1,N_platoon);
  if(N_platoon>1)
  {
 ...
 gmorph = N_GP*N_platoon*gender*nseas;
  ...
  curr_age_beg.allocate(1,gmorph,1,nseas,0,nages);
  curr_age_mid.allocate(1,gmorph,1,nseas,0,nages);
  lin_grow_beg.allocate(1,gmorph,1,nseas,0,nages);
  lin_grow_mid.allocate(1,gmorph,1,nseas,0,nages);
  first_grow_age2.allocate(1,gmorph,1,nseas);
...
echoinput<<Grow_type<<" growth model "<<endl;
 ...
 if (Grow_type==3) {k=4;}  //  min and max age for age-specific K
  ...
  if(k>0) echoinput<<tempvec5<<" growth age1 and age2"<<endl;
  k1=0;
  AFIX=0.;
  ...
  else if(Grow_type==3)
  {
    ...
    Age_K_1=tempvec5(3);
    Age_K_2=tempvec5(4);
    N_growparms=5+(Age_K_2-Age_K_1+1);
  }
  else if(Grow_type==4)
  {
...
if(k1>0) echoinput<<"  Age_Maturity"<<Age_Maturity<<endl;
 ...
 if(Fecund_Option>5) {N_warn++; cout<<" EXIT - see warning "<<endl; warning<<"Illegal fecundity option:  "<<Fecund_Option<<endl; exit(1);}
  Hermaphro_Option.allocate("Hermaphro_Option");
 echoinput<<Hermaphro_Option<<"  Hermaphro_Option "<<endl;
 MGparm_Hermaphro=0;
 k=0;
 if(Hermaphro_Option>0) k=2;
  Hermaphro_more.allocate(1,k,"Hermaphro_more");
  Hermaphro_seas=0;
  Hermaphro_maleSPB=0;
  if (k>0)
  {
 ...
 echoinput<<MG_adjust_method<<"  MG_adjust_method"<<endl;
  time_vary_MG.allocate(styr-3,YrMax+1,0,6);
  MG_active.allocate(0,6);
  ...
  for(gp=1;gp<=N_GP;gp++)
  {
    ...
    switch (Grow_type)
    {
      ...
      case 3:
      {
        ...
        ParmLabel+="VonBert_K_"+GenderLbl(gg)+"_GP_"+NumLbl(gp);
        ParCount+=3;
        for(a=Age_K_1;a<=Age_K_2;a++)
        {
          ParmLabel+="Age_K_"+GenderLbl(gg)+"_GP_"+NumLbl(gp)+"_a_"+NumLbl(a);
          ParCount++;
        }
        break;
      }
  ...
  MGparm_envuse.allocate(1,N_MGparm);
  MGparm_envtype.allocate(1,N_MGparm);
...
if(N_MGparm_env>0) echoinput<<" MGparm-env setup "<<endl<<MGparm_env_1<<endl;
  ...
  MGparm_trend_point.allocate(1,N_MGparm);
...
if(N_MGparm_blk>0) echoinput<<" MGparm-blk setup "<<endl<<MGparm_blk_1<<endl;
  ...
  MGparm_trend_1.allocate(1,N_MGparm_trend2,1,7,"MGparm_trend_1");
if(N_MGparm_trend2>0) echoinput<<"MG trend and cycle parameters "<<endl<<MGparm_trend_1<<endl;
  MGparm_trend_rev.allocate(1,N_MGparm_trend);
  MGparm_trend_rev_1.allocate(1,N_MGparm_trend);
  if(N_MGparm_trend>0)
  {
    k1=0;
...
N_MGparm2=N_MGparm+N_MGparm_env+N_MGparm_blk+N_MGparm_trend2+N_MGparm_seas;
  ...
  MGparm_dev_type.allocate(1,N_MGparm_dev);
  MGparm_dev_select.allocate(1,N_MGparm);
...
N_SRparm2=N_SRparm(SR_fxn)+4;
  SR_parm_1.allocate(1,N_SRparm2,1,7,"SR_parm_1");
...
echoinput<<SR_env_target_RD<<" SR_env_target_RD "<<endl;
  SRvec_LO.allocate(1,N_SRparm2);
  SRvec_HI.allocate(1,N_SRparm2);
  SRvec_PH.allocate(1,N_SRparm2);
...
echoinput<<recdev_end<<" recdev_end"<<endl;
  recdev_PH_rd.allocate("recdev_PH_rd");
...
echoinput<<recdev_adv<<" recdev_adv"<<endl;
  recdev_options_rd.allocate(1,13*recdev_adv,"recdev_options_rd");
  recdev_options.allocate(1,13);
  recdev_adj.allocate(1,5);
  ...
  recdev_cycle_parm_RD.allocate(1,recdev_cycle,1,14,"recdev_cycle_parm_RD");
k=1;
if(recdev_cycle>0) k=recdev_cycle;
  recdev_cycle_LO.allocate(1,k);
  recdev_cycle_HI.allocate(1,k);
  recdev_cycle_PH.allocate(1,k);
...
echoinput<<" detailed F_setups "<<endl<<F_setup2<<endl;
 ...
 echoinput<<" initial equil F parameter setup"<<endl<<init_F_parm_1<<endl;
  ...
  Q_setup.allocate(1,Ntypes,1,4,"Q_setup");
...
echoinput<<" selex parm setup "<<endl<<selparm_1<<endl;
  time_vary_sel.allocate(styr-3,endyr+1,1,2*Ntypes);
  time_vary_makefishsel.allocate(styr-3,endyr+1,1,Nfleet);
  selparm_env.allocate(1,N_selparm);
  selparm_envuse.allocate(1,N_selparm);
  selparm_envtype.allocate(1,N_selparm);
  N_selparm_env=0;
...
if(k1>0) echoinput<<" selex-env parameters "<<selparm_env_1<<endl;
  Block_Defs_Sel.allocate(1,N_selparm,styr,endyr);
  selparm_trend_point.allocate(1,N_selparm);
  ...
  selparm_trend_1.allocate(1,N_selparm_trend2,1,7,"selparm_trend_1");
if(N_selparm_trend2>0) echoinput<<"Selex trend and cycle parameters "<<endl<<selparm_trend_1<<endl;
  selparm_trend_rev.allocate(1,N_selparm_trend);
  selparm_trend_rev_1.allocate(1,N_selparm_trend);
  ...
  selparm_dev_type.allocate(1,j);
  selparm_dev_select.allocate(1,N_selparm);
...
if(Do_TG>0) {k1=k;} else {k1=1;}
  ...
  SzFreq_lambda.allocate(1,SzFreq_N_Like,1,max_lambda_phase);
   ...
   TG_lambda2=1.;  //16
    ...
    for(j=1;j<=N_lambda_changes;j++)
    {
      ...
      else if(k>16)
      {
        k=0;
        N_warn++;
        warning<<" illegal lambda_type for lambda change at row: "<<j<<" Method: "<<k<<" > 16"<<endl;
      }
  ...
  More_Std_Input.allocate(1,Do_More_Std*9,"More_Std_Input");
  ...
  NatAge_Std_Pick.allocate(1,NatAge_Std_Cnt,"NatAge_Std_Pick");
...
set_covariance_matrix(CoVar);
  ...
  WTage_in.allocate(1,N_WTage_rd,1,7+N_WTage_maxage,"WTage_in");
  junkvec2.allocate(0,nages);
  WTage_emp.allocate(styr-3*nseas,k2,1,gender*N_GP*nseas,-2,Ntypes,0,nages);
  if(k1>0)
  {
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  MGparm_trend.allocate(1,N_MGparm_trend,styr,YrMax,"MGparm_trend");
  #ifndef NO_AD_INITIALIZE
    MGparm_trend.initialize();
  #endif
  MGparm_block_val.allocate(1,N_MGparm,styr,YrMax,"MGparm_block_val");
  #ifndef NO_AD_INITIALIZE
    MGparm_block_val.initialize();
  #endif
  MGparm_dev.allocate(1,N_MGparm_dev,MGparm_dev_minyr,MGparm_dev_maxyr,-10,10,MGparm_dev_PH,"MGparm_dev");
  MGparm_dev_rwalk.allocate(1,N_MGparm_dev,MGparm_dev_minyr,MGparm_dev_maxyr,"MGparm_dev_rwalk");
  #ifndef NO_AD_INITIALIZE
    MGparm_dev_rwalk.initialize();
  #endif
  ...
  #endif
  VBK.allocate(1,N_GP*gender,0,nages,"VBK");
  #ifndef NO_AD_INITIALIZE
    VBK.initialize();
  #endif
  ...
  Lmin.allocate(1,N_GP*gender,"Lmin");
    ...
    Lmin.initialize();
  #endif
  Lmin_last.allocate(1,N_GP*gender,"Lmin_last");
  #ifndef NO_AD_INITIALIZE
    Lmin_last.initialize();
  #endif
  ...
  #endif
  Cohort_Lmin.allocate(1,N_GP*gender,styr,YrMax,0,nages,"Cohort_Lmin");
  #ifndef NO_AD_INITIALIZE
    Cohort_Lmin.initialize();
  #endif
  ...
  wt_len2.allocate(1,nseas,1,nlength2,"wt_len2");
    ...
    wt_len2.initialize();
  #endif
  wt_len2_sq.allocate(1,nseas,1,nlength2,"wt_len2_sq");
  #ifndef NO_AD_INITIALIZE
    wt_len2_sq.initialize();
  #endif
  wt_len_low.allocate(1,nseas,1,nlength2,"wt_len_low");
  #ifndef NO_AD_INITIALIZE
    wt_len_low.initialize();
  #endif
  ...
  mat_age.allocate(0,nages,"mat_age");
    ...
    mat_age.initialize();
  #endif
  Hermaphro_val.allocate(1,N_GP,0,nages,"Hermaphro_val");
  #ifndef NO_AD_INITIALIZE
    Hermaphro_val.initialize();
  #endif
  age_age.allocate(0,N_ageerr,1,n_abins2,0,gender*nages+gender-1,"age_age");
  #ifndef NO_AD_INITIALIZE
    age_age.initialize();
  #endif
  age_err.allocate(1,N_ageerr,1,2,0,nages,"age_err");
    ...
    age_err.initialize();
  #endif
  ALK.allocate(1,2*nseas,1,gmorph,0,nages,1,nlength,"ALK");
  #ifndef NO_AD_INITIALIZE
    ALK.initialize();
  #endif
  ...
  #endif
  Ave_Size.allocate(styr-3*nseas,TimeMax_Fcast_std+nseas,0,1,1,gmorph,0,nages,"Ave_Size");
  #ifndef NO_AD_INITIALIZE
    Ave_Size.initialize();
  #endif
  CV_G.allocate(1,N_GP*gender,1,2*nseas,0,nages,"CV_G");
  #ifndef NO_AD_INITIALIZE
    CV_G.initialize();
  #endif
  Save_Wt_Age.allocate(styr-3*nseas,TimeMax_Fcast_std+1,1,gmorph,0,nages,"Save_Wt_Age");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  migrrate.allocate(styr-3,endyr+1,1,do_migr2,0,nages,"migrrate");
  #ifndef NO_AD_INITIALIZE
    migrrate.initialize();
  #endif
  ...
  #endif
  SR_parm.allocate(1,N_SRparm2,SRvec_LO,SRvec_HI,SRvec_PH,"SR_parm");
  two_sigmaRsq.allocate("two_sigmaRsq");
  #ifndef NO_AD_INITIALIZE
  two_sigmaRsq.initialize();
  #endif
  half_sigmaRsq.allocate("half_sigmaRsq");
  #ifndef NO_AD_INITIALIZE
  half_sigmaRsq.initialize();
  #endif
  ...
  #endif
  ///// START NEW CODE ///// 
  // PREVIOUS CODE DIDNT WORK RIGHT BECAUSE SR_parm ISNT INITIALIZED AT THIS POINT, SO A LOGICAL CHECK OF SR_parm ISNT APPROPRIATE
  // N_SRparm2 is the number of rows of SR_parm_1 (i.e., the number of SR parameters)
  // SR_autocorr=1 if either its value isnt 0, or its phase is positive
  if(SR_parm_1(N_SRparm2,3)!=0.0 || SR_parm_1(N_SRparm2,7)>0) {
    SR_autocorr=1;
  }else{
    SR_autocorr=0;
  }  // flag for recruitment autocorrelation
  cout<<" Do recruitment AR yes/no: SR_autocorr= "<<SR_autocorr<<endl;
  echoinput<<" Do recruitment AR yes/no: SR_autocorr= "<<SR_autocorr<<endl;
  //cout<<" N_SRparm2= "<<N_SRparm2<<endl;
  //cout<<" SRvec_PH= "<<SRvec_PH<<endl;
  //cout<<" SR_parm= "<<SR_parm<<endl;
  //cout<<" SR_parm(N_SRparm2)= "<<SR_parm(N_SRparm2)<<endl;
  ///// END NEW CODE /////
  if(do_recdev==1)
  ...
  {s=1; p=-1; k=1; j=-1;}
  biasadj.allocate(styr-nages,YrMax,"biasadj");
  #ifndef NO_AD_INITIALIZE
    biasadj.initialize();
  #endif
  biasadj_full.allocate(styr-nages,YrMax,"biasadj_full");
  #ifndef NO_AD_INITIALIZE
    biasadj_full.initialize();
  #endif
  sd_offset_rec.allocate("sd_offset_rec");
  #ifndef NO_AD_INITIALIZE
  sd_offset_rec.initialize();
  #endif
  recdev_cycle_parm.allocate(1,recdev_cycle,recdev_cycle_LO,recdev_cycle_HI,recdev_cycle_PH,"recdev_cycle_parm");
  ...
  Fcast_recruitments.allocate(recdev_end+1,YrMax,recdev_LO,recdev_HI,Fcast_recr_PH2,"Fcast_recruitments");
  ...
  {k=-1;}
  Fcast_impl_error.allocate(endyr+1,YrMax,-1,1,k,"Fcast_impl_error");
  SPB_current.allocate("SPB_current");
  #ifndef NO_AD_INITIALIZE
  SPB_current.initialize();
  #endif
  SPB_vir_LH.allocate("SPB_vir_LH");
  #ifndef NO_AD_INITIALIZE
  SPB_vir_LH.initialize();
  #endif
  Recr_virgin.allocate("Recr_virgin");
  #ifndef NO_AD_INITIALIZE
  Recr_virgin.initialize();
  #endif
  SPB_virgin.allocate("SPB_virgin");
  #ifndef NO_AD_INITIALIZE
  SPB_virgin.initialize();
  #endif
  SPR_unf.allocate("SPR_unf");
  #ifndef NO_AD_INITIALIZE
  SPR_unf.initialize();
  #endif
  SPR_trial.allocate("SPR_trial");
  #ifndef NO_AD_INITIALIZE
  SPR_trial.initialize();
  #endif
  SPB_pop_gp.allocate(styr-3,YrMax,1,pop,1,N_GP,"SPB_pop_gp");
  #ifndef NO_AD_INITIALIZE
    SPB_pop_gp.initialize();
  #endif
  SPB_yr.allocate(styr-3,YrMax,"SPB_yr");
  #ifndef NO_AD_INITIALIZE
    SPB_yr.initialize();
  #endif
k=0;
if(Hermaphro_Option>0) k=1;
  MaleSPB.allocate(styr-3,YrMax*k,1,pop,1,N_GP,"MaleSPB");
  #ifndef NO_AD_INITIALIZE
    MaleSPB.initialize();
  #endif
  SPB_equil_pop_gp.allocate(1,pop,1,N_GP,"SPB_equil_pop_gp");
  #ifndef NO_AD_INITIALIZE
    SPB_equil_pop_gp.initialize();
  #endif
  MaleSPB_equil_pop_gp.allocate(1,pop,1,N_GP,"MaleSPB_equil_pop_gp");
  #ifndef NO_AD_INITIALIZE
    MaleSPB_equil_pop_gp.initialize();
  #endif
  SPB_equil.allocate("SPB_equil");
  #ifndef NO_AD_INITIALIZE
  SPB_equil.initialize();
  #endif
  SPR_temp.allocate("SPR_temp");
  #ifndef NO_AD_INITIALIZE
  SPR_temp.initialize();
  #endif
  ...
  Recruits.initialize();
  #endif
  Recr.allocate(1,pop,styr-3,YrMax,"Recr");
  #ifndef NO_AD_INITIALIZE
    Recr.initialize();
  #endif
  exp_rec.allocate(styr-3,YrMax,"exp_rec");
  #ifndef NO_AD_INITIALIZE
    exp_rec.initialize();
  #endif
  pred_rec.allocate(styr-3,YrMax,"pred_rec");
  #ifndef NO_AD_INITIALIZE
    pred_rec.initialize();
  #endif
  use_rec.allocate(styr-3,YrMax,"use_rec");
  #ifndef NO_AD_INITIALIZE
    use_rec.initialize();
  #endif
  ...
  agetemp.allocate(0,gender*nages+gender-1,"agetemp");
    ...
    agetemp.initialize();
  #endif
  Save_PopLen.allocate(styr-3*nseas,TimeMax_Fcast_std+1,1,2*pop,1,gmorph,1,nlength,"Save_PopLen");
  #ifndef NO_AD_INITIALIZE
    Save_PopLen.initialize();
  #endif
  Save_PopAge.allocate(styr-3*nseas,TimeMax_Fcast_std+1,1,2*pop,1,gmorph,0,nages,"Save_PopAge");
  #ifndef NO_AD_INITIALIZE
    Save_PopAge.initialize();
  #endif
...
if(Do_Forecast>0) {k=TimeMax_Fcast_std+1;} else {k=TimeMax+nseas;}
  natage.allocate(styr-3*nseas,k,1,pop,1,gmorph,0,nages,"natage");
  #ifndef NO_AD_INITIALIZE
    natage.initialize();
  #endif
  ...
  #endif
  Hrate.allocate(1,Nfleet,styr-3*nseas,k,"Hrate");
  #ifndef NO_AD_INITIALIZE
    Hrate.initialize();
  #endif
  catch_fleet.allocate(styr-3*nseas,TimeMax_Fcast_std,1,Nfleet,1,6,"catch_fleet");
  #ifndef NO_AD_INITIALIZE
    catch_fleet.initialize();
  #endif
  annual_catch.allocate(styr,YrMax,1,6,"annual_catch");
  #ifndef NO_AD_INITIALIZE
    annual_catch.initialize();
  #endif
  annual_F.allocate(styr,YrMax,1,2,"annual_F");
  #ifndef NO_AD_INITIALIZE
    annual_F.initialize();
  #endif
  ...
  smrynum.initialize();
  #endif
  smryage.allocate("smryage");
  #ifndef NO_AD_INITIALIZE
  smryage.initialize();
  #endif
  catch_mnage.allocate("catch_mnage");
  #ifndef NO_AD_INITIALIZE
  catch_mnage.initialize();
  #endif
  catch_mnage_d.allocate("catch_mnage_d");
  #ifndef NO_AD_INITIALIZE
  catch_mnage_d.initialize();
  #endif
  harvest_rate.allocate("harvest_rate");
  #ifndef NO_AD_INITIALIZE
  harvest_rate.initialize();
  #endif
  maxpossF.allocate("maxpossF");
  #ifndef NO_AD_INITIALIZE
  maxpossF.initialize();
  #endif
  Get_EquilCalc.allocate(1,2,"Get_EquilCalc");
  #ifndef NO_AD_INITIALIZE
    Get_EquilCalc.initialize();
  #endif
  Z_rate.allocate(styr-3*nseas,k,1,pop,1,gmorph,0,nages,"Z_rate");
  #ifndef NO_AD_INITIALIZE
    Z_rate.initialize();
  #endif
  ...
  if(F_Method==2)    // continuous F
    ...
    {k=-1;}
  F_rate.allocate(1,k,0.,50.,have_catch,"F_rate");
  ...
  Fmult.initialize();
  #endif
  Fcast_Fmult.allocate("Fcast_Fmult");
  #ifndef NO_AD_INITIALIZE
  Fcast_Fmult.initialize();
  #endif
  Fcurr_Fmult.allocate("Fcurr_Fmult");
  #ifndef NO_AD_INITIALIZE
  Fcurr_Fmult.initialize();
  #endif
  ...
  #endif
  Fcast_RelF_Use.allocate(1,nseas,1,Nfleet,"Fcast_RelF_Use");
  #ifndef NO_AD_INITIALIZE
    Fcast_RelF_Use.initialize();
  #endif
  Bmark_RelF_Use.allocate(1,nseas,1,Nfleet,"Bmark_RelF_Use");
  #ifndef NO_AD_INITIALIZE
  ...
  log_q_cr.allocate(1,Ntypes,1,nyr_cr,"log_q_cr");
    ...
    log_q_cr.initialize();
  #endif
  q_cr.allocate(1,Ntypes,1,nyr_cr,"q_cr");
  #ifndef NO_AD_INITIALIZE
    q_cr.initialize();
  #endif
  se_cr_use.allocate(1,Ntypes,1,nyr_cr,"se_cr_use");
  #ifndef NO_AD_INITIALIZE
    se_cr_use.initialize();
  #endif
  ...
  exp_disc.allocate(1,Ntypes,1,nyr_disc,"exp_disc");
    ...
    exp_disc.initialize();
  #endif
  retain.allocate(styr-3,endyr+1,1,Ntypes,1,nlength2,"retain");
  #ifndef NO_AD_INITIALIZE
    retain.initialize();
  #endif
  retain_M.allocate(1,nlength,"retain_M");
    ...
    retain_M.initialize();
  #endif
  discmort.allocate(styr-3,endyr+1,1,Ntypes,1,nlength2,"discmort");
  #ifndef NO_AD_INITIALIZE
    discmort.initialize();
  #endif
  ...
  #endif
  SzFreqTrans.allocate(1,SzFreq_Nmeth*nseas,1,nlength2,1,SzFreq_Nbins_seas_g,"SzFreqTrans");
  #ifndef NO_AD_INITIALIZE
    SzFreqTrans.initialize();
  #endif
  selparm.allocate(1,N_selparm2,selparm_LO,selparm_HI,selparm_PH,"selparm");
  selparm_trend.allocate(1,N_selparm_trend,styr,endyr,"selparm_trend");
  #ifndef NO_AD_INITIALIZE
    selparm_trend.initialize();
  #endif
  selparm_block_val.allocate(1,N_selparm,styr,endyr,"selparm_block_val");
  #ifndef NO_AD_INITIALIZE
    selparm_block_val.initialize();
  #endif
  selparm_dev.allocate(1,N_selparm_dev,selparm_dev_minyr,selparm_dev_maxyr,-10,10,selparm_dev_PH,"selparm_dev");
  selparm_dev_rwalk.allocate(1,N_selparm_dev,selparm_dev_minyr,selparm_dev_maxyr,"selparm_dev_rwalk");
  #ifndef NO_AD_INITIALIZE
    selparm_dev_rwalk.initialize();
  #endif
  sel_l.allocate(styr-3,endyr+1,1,Ntypes,1,gender,1,nlength,"sel_l");
  #ifndef NO_AD_INITIALIZE
    sel_l.initialize();
  #endif
  sel_l_r.allocate(styr-3,endyr+1,1,Ntypes,1,gender,1,nlength,"sel_l_r");
  #ifndef NO_AD_INITIALIZE
    sel_l_r.initialize();
  #endif
  discmort2.allocate(styr-3,endyr+1,1,Ntypes,1,gender,1,nlength,"discmort2");
  #ifndef NO_AD_INITIALIZE
    discmort2.initialize();
  #endif
  sel_a.allocate(styr-3,endyr+1,1,Ntypes,1,gender,0,nages,"sel_a");
  #ifndef NO_AD_INITIALIZE
    sel_a.initialize();
  #endif
  ...
  {k=TimeMax+nseas;}
  fish_body_wt.allocate(styr-3*nseas,k,1,gmorph,1,Nfleet,0,nages,"fish_body_wt");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  save_sel_fec.allocate(styr-3*nseas,TimeMax_Fcast_std+nseas,1,gmorph,0,Nfleet,0,nages,"save_sel_fec");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  save_sp_len.allocate(styr,endyr+1,1,2*Ntypes,1,50,"save_sp_len");
  #ifndef NO_AD_INITIALIZE
    save_sp_len.initialize();
  #endif
  exp_l.allocate(1,Ntypes,1,Nobs_l,1,nlen_bin2,"exp_l");
  #ifndef NO_AD_INITIALIZE
    exp_l.initialize();
  #endif
  neff_l.allocate(1,Ntypes,1,Nobs_l,"neff_l");
  #ifndef NO_AD_INITIALIZE
    neff_l.initialize();
  #endif
  tempvec_l.allocate(1,nlength,"tempvec_l");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  SzFreq_exp.allocate(1,SzFreq_totobs,1,SzFreq_Setup2,"SzFreq_exp");
  #ifndef NO_AD_INITIALIZE
    SzFreq_exp.initialize();
  #endif
  SzFreq_like.allocate(1,SzFreq_N_Like,"SzFreq_like");
  #ifndef NO_AD_INITIALIZE
    SzFreq_like.initialize();
  #endif
  exp_a.allocate(1,Ntypes,1,Nobs_a,1,n_abins2,"exp_a");
  #ifndef NO_AD_INITIALIZE
    exp_a.initialize();
  #endif
  ...
  #endif
  tempvec_a.allocate(0,nages,"tempvec_a");
  #ifndef NO_AD_INITIALIZE
    tempvec_a.initialize();
  #endif
  neff_a.allocate(1,Ntypes,1,Nobs_a,"neff_a");
  #ifndef NO_AD_INITIALIZE
    neff_a.initialize();
  #endif
  ...
  #endif
  exp_ms.allocate(1,Ntypes,1,Nobs_ms,1,n_abins2,"exp_ms");
  #ifndef NO_AD_INITIALIZE
    exp_ms.initialize();
  #endif
  exp_ms_sq.allocate(1,Ntypes,1,Nobs_ms,1,n_abins2,"exp_ms_sq");
  #ifndef NO_AD_INITIALIZE
  ...
  #endif
  recr_like.allocate("recr_like");
  #ifndef NO_AD_INITIALIZE
  recr_like.initialize();
  #endif
  Fcast_recr_like.allocate("Fcast_recr_like");
  #ifndef NO_AD_INITIALIZE
  ...
  temp2.initialize();
  #endif
  temp3.allocate("temp3");
  #ifndef NO_AD_INITIALIZE
  temp3.initialize();
  #endif
  temp4.allocate("temp4");
  #ifndef NO_AD_INITIALIZE
  temp4.initialize();
  #endif
  ...
  #endif
  Smry_Table.allocate(styr-3,YrMax,1,20+2*gmorph,"Smry_Table");
  #ifndef NO_AD_INITIALIZE
  ...
  TG_save.allocate(1,N_TG,1,3+TG_endtime,"TG_save");
    ...
    TG_save.initialize();
  #endif
  SPB_std.allocate(1,N_STD_Yr,"SPB_std");
  ...
  Mgmt_quant.allocate(1,N_STD_Mgmt_Quant,"Mgmt_quant");
  Extra_Std.allocate(1,Extra_Std_N,"Extra_Std");
  ...
  #endif
  SR_parm_Like.allocate(1,N_SRparm2,"SR_parm_Like");
  #ifndef NO_AD_INITIALIZE
    SR_parm_Like.initialize();
  #endif
  recdev_cycle_Like.allocate(1,recdev_cycle,"recdev_cycle_Like");
  #ifndef NO_AD_INITIALIZE
    recdev_cycle_Like.initialize();
  #endif
 k=Do_TG*(3*N_TG+2*Nfleet);
  ...
  if(mceval_counter==0 || (mceval_counter>burn_intvl &&  ((double(mceval_counter)/double(thin_intvl)) - double((mceval_counter/thin_intvl))==0)  )) // check to see if burn in period is over
  {
    ...
    if(rundetail>1)
     ...
     {cout<<" MCeval: "<<mceval_counter<<" -log(L): "<<obj_fun<<"  Spbio: "<<value(SPB_yr(styr))<<" "<<value(SPB_yr(endyr));}
       if(F_ballpark_lambda>0. && F_ballpark_like>0.01) cout<<" F_like " <<F_ballpark_like;
...
void model_parameters::report(const dvector& gradients)
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  report<<"Virg "<<SPB_yr(styr-2)<<" "<<use_rec(styr-2)<<endl;
  report<<"Init "<<SPB_yr(styr-1)<<" "<<use_rec(styr-1)<<endl;
  for(y=styr;y<=endyr;y++) report<<y<<" "<<SPB_yr(y)<<" "<<use_rec(y)<<endl;
    ...
    int k2;
      ...
      for (gp=1;gp<=N_GP*gender;gp++)
      {
        ...
        if(Grow_type==3)  //  age specific k
        {
          for(a=Age_K_1;a<=Age_K_2;a++)
          {
            VBK(gp,a)-=mgp_adj(Ip+3+a-Age_K_1);
          }
        }
        k=min(nages,(YrMax-y));
        if(y==styr)
          ...
          if(use_morph(g)>0)
          {
            if(y==styr)
            {
              switch(Grow_type)
              {
                default:
                {
                  if(do_once==1) echoinput<<" growth "<<gp<<" "<<g<<" "<<Lmin(gp)<<" "<<L_inf(gp)<<" "<<-VBK(gp)<<endl;
                  ...
                  if(do_once==1) echoinput<<" avesize_w/o_linear_section "<<Ave_Size(styr,0,g)<<endl;
                  if(birthseas>1) Ave_Size(styr,0,g,0)=len_bins(1);
                  break;
                }
                case 2:  //  Richards; age-specific VBK not allowed here
                {
                  if(do_once==1) echoinput<<" growth "<<gp<<" "<<g<<" "<<LminR<<" "<<LinfR<<" "<<-VBK(gp,nages)<<endl<<"Avesize (ages 0 to 6): "<<Ave_Size(styr,0,g)(0,min(6,nages))<<endl;
                  for(a=0;a<=nages;a++)
                  {
            ...
            for (s=1;s<=nseas;s++)
            {
              ...
              switch(Grow_type)
              {
                ...
                case 2:   //  Richards
                {
                  ...
                  if(docheckup==1) checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" Lmin: "<<Lmin(gp)<<" Linf: "<<L_inf(gp)<<" VBK: "<<VBK(gp,nages)<<endl
              ...
              if(docheckup==1)
              {
              checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" size_beg "<<Ave_Size(t,0,g)<<endl;
              checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" CV_g_beg "<<CV_G(gp,s)<<endl;
              checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" sd_beg "<<Sd_Size_within(s,g)<<endl;
              checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" size_mid "<<Ave_Size(t,1,g)<<endl;
              checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" CV_g_mid "<<CV_G(gp,smid)<<endl;
              checkup<<y<<" "<<s<<" "<<g<<" "<<gp<<" sd_mid "<<Sd_Size_within(smid,g)<<endl;
              }
            }
            k=y+1;
            j=yz+1;
    ...
    if(s==spawn_seas)  // get biology of maturity and fecundity
    {
      ...
      else
      {
        ...
        switch (Fecund_Option)
        {
          ...
          case 4:
          {       // as eggs = a + b*Len
            fec_len = wtlen_p(5) + wtlen_p(6)*len_bins_m;
            if(wtlen_p(5)<0.0)
            {
              z=1;
              while(fec_len(z)<0.0)
  ...
  for (f=1;f<=2*Ntypes;f++)  // loops twice, first for size then for age
  {
    ...
    {    // recalculate the selex in this year x type
      if(N_selparmvec(f)>0)      // type has parms, so look for adjustments
      {
        ...
        if(docheckup==1) checkup<<" selex parms "<<f<<" "<<endl<<sp(1,N_selparmvec(f))<<endl;
  ...
  */
    ...
    // ***************************************
        ...
        if(gg==2)         // males exist and am now in the male loop
        {
         ...
         else if(seltype(f,3)!=3 && seltype(f,3)!=4 &&seltype(f,1)!=5)
         {
      ...
      if(docheckup==1) checkup<<"sel-len "<<sel_l(y,f)<<endl;
        ...
        // discmort2 is size-specific fraction that die from being retained or are dead discard = elem_prod(sel,(retain + (1-retain)*discmort))
      ...
      else
      {
        ...
        else  // use parameters
        {
          ...
          if(docheckup==1&&y==styr) checkup<<"parms "<<sp(k)<<" "<<sp(k+1)<<" "<<sp(k+3)<<" "<<temp1<<endl<<"maleoff "<<male_offset<<endl;
          if(docheckup==1&&y==styr) checkup<<"lenbins "<<len_bins_m2<<endl;
  ...
  // Do male relative to female selex
      ...
      //            sel_a(y,fs)/=temp1;
            ...
            if(docheckup==1) checkup<<" sel-age "<<sel_a(y,fs)<<endl;
          }
        }
...
void model_parameters::get_initial_conditions(void)
{
  ...
  exp_rec(eq_yr)=Recr_virgin;                 //  expected Recr from s-r parms
  pred_rec(eq_yr)=Recr_virgin;
  use_rec(eq_yr)=Recr_virgin;
   bio_yr=styr;
   Fishon=0;
...
void model_parameters::get_time_series(void)
{
    ...
    for (s=1;s<=nseas;s++)
    {
      ...
      for (p=1;p<=pop;p++)     //loop over populations      ProgLabel_104
      {
        ...
        if(use_morph(g)>0)
        {
          ...
          if(docheckup==1) checkup<<p<<" "<<g<<" "<<GP3(g)<<" area & morph "<<endl<<"N-at-age "<<natage(t,p,g)(0,min(6,nages))<<endl
            ...
            case 1:          // F_Method is Pope's approximation
            {
              ...
              if (catch_seas_area(t,p,f)==1)
              {
                ...
                if(use_morph(g)>0)
                {
                  ...
                  if(docheckup==1) checkup<<"killrate "<<deadfish(s,g,f)(0,min(6,nages))<<endl;
                  if(catchunits(f)==1)
              ...
              if(use_morph(g)>0)
              {
                ...
                if(docheckup==1) checkup<<"total catch-at-age for morph "<<g<<" "<<catage_tot(g)(0,min(6,nages))<<endl;
              }
              break;
            }   //  end Pope's approx
            ...
            if(use_morph(g)>0)
            {
              ...
              else   //  continuous F
              {
                if(timing<0.0) // mimic fishery catch, but without Hrate so get available numbers
                {
                  for (a=0;a<=nages;a++)
                  {
                    temp=natage(t,p,g,a)*Zrate2(g,a)*sel_a(y,f,gg,a);
                    if(dolen(f)==1)
                    {pre_AL.shift(1)=temp*elem_prod(ALK(smid,g,a),sel_l(y,f,gg));}
                    else
                    {pre_AL.shift(1)=temp*ALK(smid,g,a);}
        ...
        if(SzFreq_HaveObs2(SzFreqMethod,t)==f)  // first occurrence of this method at this time is with fleet = f
        {
          if(do_once==1 || (MG_active(3)>0 && (time_vary_MG(y,3)>0 )))  // calc the matrix because it may have changed
          {
            for(gg=1;gg<=gender;gg++)
            {
                ...
                case(2):   // units are numbers
                {
                  if(SzFreq_scale(SzFreqMethod)<=2)   //  bin demarcations are in weight units (1=kg, 2=lbs), so uses wt_len to compare to bins
                  {
                    ...
                    for(z=z1;z<=z2;z++)
                    {
                      ...
                      {
                        if(wt_len_low(s,z)>=topbin) {ibin++; ibinsave++;}  //checkup<<" incr ibin "<<z<<" "<<ibin<<" "<<len_bins(z)<<" "<<len_bins_dat(ibin);
                        if(ibin>1)  {botbin=SzFreq_bins2(SzFreqMethod,ibin);}
                        ...
                        {
                          topbin=SzFreq_bins2(SzFreqMethod,ibin+1);
                          if(wt_len_low(s,z)>=botbin && wt_len_low(s,z+1)<=topbin )  //checkup<<" pop inside dat, put here"<<endl;
                          {SzFreqTrans(SzFreqMethod_seas,z,ibinsave)=1.;}
              ...
              if(docheckup==1 && gg==gender) checkup<<" sizefreq trans_matrix: method/season "<<SzFreqMethod<<" / "<<s<<endl
       ...
       if(yr_cr2(f,t)>0)
       {
         ...
         if (seltype(f,1)>=30)
         {
           switch(seltype(f,1))
           {
             ...
             {vbio=SPB_current*mfexp(recdev(y)); break;}
       ...
       if(nobs_ms_tot>0)
       {
       ...
       }   // end size-at-age
        {
         ...
         if(yr_a_t(f,i)==t)
         {
          ...
          if(docheckup==1) checkup<<" age obs2 "<<mkt_a(f,i)<<endl<<obs_a(f,i)<<endl<<exp_a(f,i)<<endl;
        ...
        if(use_morph(g)>0)
        {
          ...
          if(docheckup==1)
          {
            ...
            if(F_Method>1) checkup<<"Z-at-age "<<Z_rate(t,p,g)(0,min(6,nages))<<endl;
            checkup<<" survivors "<<natage(t+1,p,g)(0,min(6,nages))<<endl;
          }
        } // close gmorph loop
      } //close area loop
      if(Hermaphro_Option>0)
  ...
  if(docheckup==1) checkup<<" TG_"<<TG<<" y_"<<y<<" s_"<<s<<" area_"<<p<<" g_"<<g<<" GP3_"<<GP3(g)<<" f_"<<f<<" a1_"<<a1<<" Sel_"<<Sel_for_tag(t,g,f,a1)<<" TG_alive_"<<TG_alive(p,g)<<" TG_obs_"<<TG_recap_obs(TG,TG_t,f)<<" TG_exp_"<<TG_recap_exp(TG,TG_t,f)<<endl;
          ...
          if(do_migration>0)  //  movement between areas of tags
          {
            ...
            if(docheckup==1) checkup<<" Tag_alive after survival and movement "<<endl<<TG_alive<<endl;
          }
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  if(Nobs_a_tot>0)
  {
    ...
    if(recrdev_lambda(k_phase)>0.0 && (do_recdev>0 || recdev_do_early>0) )
    {
      ...
      if(SR_autocorr==0){
        recr_like = sd_offset_rec * log(sigmaR);     //# styr-nages,YrMax
        ...
        recr_like = sd_offset_rec * log(sqrt(1-rho*rho)*sigmaR);
        recr_like += square(recdev(recdev_first)) / ((1.0-rho*rho)*two_sigmaRsq);
    ...
    // recdev_end -- last year of main recr_devs; forecast devs start in following year (in CTL file)
    // endyr -- end year of model (in DAT file)
    // YrMax -- end year of model (in DAT file) + number of forecast years (in FORECAST file)
    if(Do_Forecast>0)
    {
      if(recdev_end<endyr){
      ...
      if(SR_autocorr==0){
        ...
        rho=SR_parm(N_SRparm2);
        Fcast_recr_like += square(Fcast_recruitments(endyr+1)-rho*recdev(recdev_end)) / ((1.0-rho*rho)*two_sigmaRsq);
        for(y=endyr+2;y<=YrMax;y++){
...
void model_parameters::Report_Parm(const int NParm, const int AC, const int Activ, const prevariable& Pval, const double& Pmin, const double& Pmax, const double& RD, const double& PR, const int PR_T, const double& CV, const int PH, const prevariable& Like)
{
  {
...
{
  {
    ...
    switch (T)
    {
      ...
      case 3: // lognormal
      {
        ...
        {N_warn++; warning<<" cannot do prior in log space for parm with min <=0.0"<<endl;}
        break;
      }
    }
    return Prior_Like;
        ...
        if(recr_dist_pattern_2(gpattern,birthseas)>0)
        {
          for(gp2=1;gp2<=N_platoon;gp2++)      // loop the platoons
          {
            ...
            if(s==spawn_seas && gg==1)  // get fecundity at age from ALK and length-spec biology
            {
              ...
              {
                fec(g) = ALK_w_beg*mat_len_wt;  //  need to adjust for growth to spawn_time_seas; perhaps use cubic spline??
                if(Maturity_Option==3)
...
void model_parameters::Get_Forecast(void)
{
  ...
  if(show_MSY==1)  //  write more headers
  {
    ...
    if(N_Fcast_catches>0)
    {
      if(Fcast_InputCatch_Basis==3)
      {report5<<"3:_Input_fixed_catches_as_retained_catch"<<endl;}
      else if (Fcast_InputCatch_Basis==2)
      {report5<<"2:_Input_fixed_catches_as_total_dead_catch"<<endl;}
      else if (Fcast_InputCatch_Basis==99)
      {report5<<"99:_Input_as_annualized_Hrate(F)"<<endl;}
    }
      ...
      for(int ABC_Loop=ABC_Loop_start; ABC_Loop<=ABC_Loop_end;ABC_Loop++)
      {
        ...
        for(s=1;s<=nseas;s++)
        {
          ...
          if(s==spawn_seas)    //  get spawnbio in a forecast year
          {
            ...
            {
              Recruits=exp_rec(y);
              use_rec(y)=exp_rec(y);  // within the spawn_recr function this has value with recrdev, so need to reset here
            }
          ...
          for(p=1;p<=pop;p++)  //  loop areas (sub-pops)
          {
            ...
            {
              gg=sx(g);
              if(s==Bseas(g)) {natage(t,p,g,0) = Recruits*recr_dist(GP(g),s,p)*submorphdist(GP2(g));} //get recruits from total recruits and RecrDist
            ...
            for(f=1;f<=Nfleet;f++)
            {
              switch (ABC_Loop)
              {
                ...
                case 2:
                {
                  ...
                  if(Fcast_InputCatch(t,f)>-1.0)  //  have an input
                  {
                    if(Fcast_InputCatch_Basis<=3)  //  input is catch
                      {
                        if(Fcast_InputCatch(t,f)==0.0)
                        {
                          Hrate(f,t)=0.0;
                          Do_F_tune(t,f)=0;
                        }
                      ...
                      {Hrate(f,t)=Fcast_InputCatch(t,f);}  // input is as Hrate (F), but do not need tuning
                  }
                  break;
                }
                  ...
                  if(Do_F_tune(t,f)==1)  // have an input catch, so get expected catch from F and Z
                  {
                    if(ABC_Loop==2)  //  tune to input catch
                    {
                      ...
                      if(use_morph(g)>0)
                      {
                        ...
                        {
                          if(Fcast_InputCatch_Basis==2)
                            {temp+=Nmid(g)*deadfish_B(s,g,f);}      // dead catch bio
                          else if(Fcast_InputCatch_Basis==3)
                      ...
                      temp=max_harvest_rate-Fcast_InputCatch(t,f)/(temp+NilNumbers);
                  ...
                  if(Do_F_tune(t,f)==1)  // have an input catch, so get expected catch from F and Z
                  {
                    if(ABC_Loop==2)  //  tune to input catch
                    {
                      ...
                      if(use_morph(g)>0)
                      {
                        ...
                        {
                          if(Fcast_InputCatch_Basis==2)
                            {temp+=elem_prod(natage(t,p,g),deadfish_B(s,g,f))*Zrate2(g);}      // dead catch bio
                          else if(Fcast_InputCatch_Basis==3)
                            {temp+=elem_prod(natage(t,p,g),sel_al_2(s,g,f))*Zrate2(g);}      // retained catch bio
                        }
                        else   //  catch in numbers
                        {
                          if(Fcast_InputCatch_Basis==2)
                          {temp+=elem_prod(natage(t,p,g),deadfish(s,g,f))*Zrate2(g);}      // deadfish catch numbers
                          else if(Fcast_InputCatch_Basis==3)
                          {temp+=elem_prod(natage(t,p,g),sel_al_4(s,g,f))*Zrate2(g);}      // retained catch numbers
                        }
                      }  //close gmorph loop
                      temp*=Hrate(f,t);
                      H_temp(f)=Hrate(f,t);
                      ...
                      if(Tune_F<3)
                      {
                        C_old(f)=C_temp(f);
                        H_old(f)=H_temp(f);
                        Hrate(f,t)*=(Fcast_InputCatch(t,f)+1.0)/(temp+1.0);  //  apply adjustment
                      }
                      else
                      {
                        Hrate(f,t)=H_old(f)+(H_temp(f)-H_old(f))/(C_temp(f)-C_old(f)+1.0e-6) * (Fcast_InputCatch(t,f)-C_old(f));
                        C_old(f)=C_temp(f);
                        H_old(f)=H_temp(f);
                      }
                    }
           ...
           if(show_MSY==1)
           {
            ...
            {report5<<" - - - - - - - ";}
            if(Fcast_InputCatch(t,f)<0.0) {report5<<" R ";} else {report5<<" C ";}
            }
...
void model_parameters::write_nucontrol(void)
{
  ...
  NuFore<<N_Fcast_catches<<" # Number of forecast catch levels to input (else calc catch from forecast F) "<<endl;
  ...
  NuFore<<"#Year Seas Fleet Catch(or_F) "<<endl;
  NuFore<<Fcast_InputCatch_1<<endl;
  ...
  report4<<natM_type<<" #_natM_type:_0=1Parm; 1=N_breakpoints;_2=Lorenzen;_3=agespecific;_4=agespec_withseasinterpolate"<<endl;
    ...
    if(Grow_type==3)
      {report4<<Age_K_1<<" "<<Age_K_2<<" # min and max age for age-specific K"<<endl;}
  ...
  report4<<"#"<<endl;
  report4<<"#Fishing Mortality info "<<endl<<F_ballpark<<" # F ballpark for tuning early phases"<<endl;
 ...
 */
  ...
  recdev_adj(1)<<" "<<recdev_adj(2,5)<<" breakpoints_for_bias_adjustment_ramp "<<endl;
   ...
   {SS2out<<endl;}
  SS2out<<"year spawn_bio exp_recr with_env adjusted pred_recr dev biasadj era"<<endl;
  SS2out<<"S/Rcurve "<<SPB_virgin<<" "<<Recr_virgin<<endl;
  SS2out<<"Virg "<<SPB_yr(styr-2)<<" "<<exp_rec(styr-2)<<" "<<exp_rec(styr-2)<<" "<<pred_rec(styr-2)<<" "<<use_rec(styr-2)<<" - "<<0.0<<" Virg"<<endl;
  SS2out<<"Init "<<SPB_yr(styr-1)<<" "<<exp_rec(styr-1)<<" "<<exp_rec(styr-1)<<" "<<pred_rec(styr-1)<<" "<<use_rec(styr-1)<<" - "<<0.0<<" Init"<<endl;
  if(recdev_first<styr)
  {
    for (y=recdev_first;y<=styr-1;y++)
    {
      SS2out<<y<<" "<<SPB_yr(styr-1)<<" "<<exp_rec(styr-1)<<" "<<exp_rec(styr-1)<<" "<<exp_rec(styr-1)*mfexp(-biasadj(y)*half_sigmaRsq)<<" "<<
      exp_rec(styr-1)*mfexp(recdev(y)-biasadj(y)*half_sigmaRsq)<<" "
     ...
     if(nsamp_a(f,i)>0)
       {
        ...
        Hrmse(f)+=value(1./neff_a(f,i)); Rrmse(f)+=value(neff_a(f,i)/nsamp_a(f,i));
         ...
         s=yr_a_t(f,i)-styr+1-(yr_a_y(f,i)-styr)*nseas;
  ...
  SS2out <<endl<< "BIOLOGY "<<sum(use_morph)<<" "<<nlength<<" "<<nages<<" "<<nseas<<" N_Used_morphs;_lengths;_ages;_season;_by_season_in_endyr" << endl;
   ...
   SS2out<<"Biology_at_age_in_endyr_with_";
   switch(CV_depvar)
   {
   case 0:
   ...
   SS2out<<endl;
   SS2out<<"Seas Morph Bio_Pattern Gender BirthSeas SubMorph Age Age_Beg Age_Mid M Len_Beg Len_Mid SD_Beg SD_Mid Wt_Beg Wt_Mid Len_Mat Age_Mat Mat*Fecund";
   ...
   for (s=1;s<=nseas;s++)
   {
     ...
     for(a=0;a<=nages;a++)
     {
      ...
      <<Wt_Age_beg(s,g,a)<<" "<<Wt_Age_mid(s,g,a)<<" "<<ALK(s,g,a)*mat_len<<" ";
      if(Maturity_Option<=2)
        {SS2out<<mat_age(a);}
    ...
    for (z=nlength;z>=1;z--)
     {
      SS2out << len_bins2(z) << " ";
      for (a=0;a<=nages;a++)
        SS2out << ALK(smid,g,a,z) << " " ;
      SS2out<<endl;
     }
      SS2out<<"mean " << Ave_Size(t,1,g) << endl;
    ...
    if(Do_TG>0)
    {
      for(TG=1;TG<=N_TG;TG++)
      {
        ...
        for(TG_t=0;TG_t<=TG_endtime(TG);TG_t++)
        {
          ...
          if(TG_t>=TG_mixperiod) {SS_compout<<1<<endl;} else {SS_compout<<0<<endl;}
          if(Nfleet>1)
          for(f=1;f<=Nfleet;f++)
          {
            SS_compout<<temp<<" "<<s<<" "<<f<<" "<<TG<<" "<<TG_release(TG,6)<<" TAG1 NA NA NA NA NA "<<
...
dvariable model_parameters::Join_Fxn(const prevariable& MinPoss, const prevariable& MaxPoss, const prevariable& Inflec, const prevariable& Xvar, const prevariable& Y1, const prevariable& Y2)
{
  {
  dvariable Yresult;
  dvariable join;
...
dvariable model_parameters::Spawn_Recr(const prevariable& SPB_current)
{
  {
    ...
    switch(SR_fxn)
    {
      ...
      case 7:  // survival based, so constrained such that recruits cannot exceed fecundity
      {
        ...
        exp_rec(y)=NewRecruits;   // expected arithmetic mean recruitment
        ...
        pred_rec(y)=SPB_curr_adj*SRZ_surv;
        if(y <=recdev_end)
        {
...
dvar_vector model_parameters::Equil_Spawn_Recr_Fxn()
{
  {
...
void model_parameters::get_age_age(const int Keynum, const int AgeKey_StartAge, const int AgeKey_Linear1, const int AgeKey_Linear2)
{
  {
