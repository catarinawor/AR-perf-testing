...
for(spp in cases.spp){
    xx <- droplevels(subset(scalars, species == spp & E == cases.E,
                            select = c(SR_LN_R0_re, E, species, bias.tried)))
    ## Need to sort manually
    xx$species <- factor(as.character(xx$species), levels = rev(cases.spp))
    ...
    with(subset(xx, bias.tried == 0),
         boxplot(SR_LN_R0_re ~ species, names = NA, las = 1, border = 1,
                    horizontal = TRUE, ylim = ylim, varwidth = TRUE))
    add.label("(a)", cex = 1.2)
    axis(2, at = 1:length(cases.spp), labels = rev(names.spp), 
